...
@section('content')
    ...
    {!! Form::model($patient_registration, ['method' => 'PUT', 'route' => ['admin.patient_registrations.update', $patient_registration->id]]) !!}
        ...
        <div class="panel-body">
            <table class="table table-bordered table-striped">
                ...
                <tbody id="message-mapping">
                    @forelse(old('message_mappings', []) as $index => $data)
                        @include('admin.patient_registrations.message_mappings_row', [
                            'index' => $index
                        ])
                    @empty
                        @foreach($patient_registration->message_mappings as $item)
                            @include('admin.patient_registrations.message_mappings_row', [
                                'index' => 'id-' . $item->id,
                                'field' => $item
                            ])
