...
@section('content')
    ...
    {!! Form::model($ppn_payment, ['method' => 'PUT', 'route' => ['admin.ppn_payments.update', $ppn_payment->id]]) !!}
...
@section('javascript')
    <script>
        $("#on_total_bill").on('change', function () {
            if($(this).val()=='Yes') {
                $("#eligible_bill_amount").val($("#total_bill").val());
            }
            else {
                Math.round(("#eligible_bill_amount").val($("#total_bill").val() - $("#total_pharmacy").val() - $("#total_consumable").val()),0);
            }
            calculate_amount();
        });

        $("#total_bill").on('change', function () {
            calculate_amount();
        });

        $("#total_pharmacy").on('change', function () {
            calculate_amount();
        });

        $("#total_consumable").on('change', function () {
            calculate_amount();
        });
        ...
        function calculate_amount() {
            if($('#type').val()=='Percentage') {
                ...
                Math.round($("#amount").val($("#eligible_bill_amount").val() * $("#percentage").val() / 100),0);
            } else {
                //$("#percentage").val(0);
                $("#percentage").attr('disabled',true);
            }
            if($("#percentage").val()==5) {
                $("#amount").val(Math.max($("#amount").val(), 25000));
            }
        }
    </script>
@endsection
